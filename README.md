...
```text
Minpro-2-PBO-SistemPengelolaanPerikanan/
├── main/
│   └── Main.java
├── model/
│   ├── Ikan.java
...
* `model` digunakan untuk menyimpan class yang berhubungan dengan data program.
* `view` digunakan untuk menampilkan menu dan menerima input dari pengguna.
* `controller` digunakan untuk mengatur proses pengolahan data seperti tambah, cari, ubah, dan hapus.
* `main` berisi `Main.java` yang digunakan untuk menjalankan program dan menghubungkan `view` dengan `controller`.
...
Berikut merupakan screenshot saat program dijalankan:
